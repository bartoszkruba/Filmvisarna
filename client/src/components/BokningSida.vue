    ...
    <section v-else>
      ...
      <div class="text">
        ...
        <div class="slutför btn">
          <button
            v-on:click="visaFelMedellande"
            type="button"
            class="btn btn-danger"
          >Slutför bokning</button>
  ...
  methods: {
    ...
    movieID() {
      if (window.location.hash.indexOf("?") > 0)
        return window.location.hash.substr(
    ...
    async bokaBiljett(){
            console.log(this.movieID());
              const ticket = {
                 movieSessionID: this.movieID(),
                  adults: 2,
                  children: 2,
                  pensioner: 2
              }
              const userCredentials = this.$store.getters.getCredentials;
              const response = await api.setTickets(ticket , userCredentials);
